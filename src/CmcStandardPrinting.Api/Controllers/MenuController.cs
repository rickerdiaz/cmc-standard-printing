...
using CmcStandardPrinting.Domain.Menus;
...
public class MenuController : ControllerBase
{
    ...
    public ActionResult<MenuData> GetMenu(int codeliste, int codesite, int codetrans, int codesetprice, int codenutrientset, int codeuser = 0, int rolelevel = 0)
    {
        ...
        try
        {
            ...
            using (var dr = cmd.ExecuteReader())
            {
                ...
                if (dr.HasRows)
                {
                    var attachments = new List<MenuAttachment>();
                    while (dr.Read())
                    {
                        attachments.Add(new MenuAttachment
                        {
                            Id = GetInt(dr["Id"]),
                            Name = GetStr(dr["Name"]),
                ...
                if (dr.HasRows)
                {
                    var calculation = new List<MenuCalculation>();
                    while (dr.Read())
                    {
                        var calc = new MenuCalculation
                        {
                            Id = GetInt(dr["Id"]),
                ...
                if (dr.HasRows)
                {
                    var nutrients = new List<MenuNutrition>();
                    while (dr.Read())
                    {
                        nutrients.Add(new MenuNutrition
                        {
                            Id = GetInt(dr["Id"]),
                ...
                dr.NextResult();
                var ingredientTranslation = new List<MenuIngredientTranslation>();
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        ingredientTranslation.Add(new MenuIngredientTranslation
                        {
                            ItemId = GetInt(dr["ID"]),
                ...
                if (dr.HasRows)
                {
                    var ingredient = new List<MenuIngredient>();
                    while (dr.Read())
                    {
                        var it = new MenuIngredient
                        {
                            CodeListe = codeliste,
                ...
                dr.NextResult();
                var procedureTranslation = new List<MenuProcedureTranslation>();
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        procedureTranslation.Add(new MenuProcedureTranslation
                        {
                ...
                if (dr.HasRows)
                {
                    var procedure = new List<MenuProcedure>();
                    while (dr.Read())
                    {
                        procedure.Add(new MenuProcedure
                        {
                            NoteId = GetInt(dr["NoteId"]),
                            Position = GetInt(dr["Position"]),
                            Note = GetStr(dr["Note"]),
                ...
                if (dr.HasRows)
                {
                    var comments = new List<MenuComment>();
                    while (dr.Read())
                    {
                        comments.Add(new MenuComment
                        {
                ...
                if (dr.HasRows)
                {
                    var proceduretemplates = new List<ProcedureTemplateInfo>();
                    while (dr.Read())
                    {
                        proceduretemplates.Add(new ProcedureTemplateInfo
                        {
                            Code = GetInt(dr["Code"]),
                            Name = GetStr(dr["Name"]),
        ...
        {
            _logger.LogWarning(ex, "Invalid menu request");
            return BadRequest();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error while loading menu");
