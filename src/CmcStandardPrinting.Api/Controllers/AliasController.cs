...
public class AliasController : ControllerBase
{
    ...
    public AliasController(IConfiguration configuration, ILogger<AliasController> logger)
    {
    ...
    public async Task<ActionResult<AliasData>> GetAlias(int codealias, int codetrans, CancellationToken ct)
    {
        try
        {
            ...
            var result = new AliasData();
            if (await rdr.ReadAsync(ct))
            {
                result.Info.Code = GetInt(rdr, "Code");
                result.Info.IdMain = GetInt(rdr, "IdMain");
                result.Info.CodeTrans = GetInt(rdr, "CodeTrans");
                result.Info.Name = GetString(rdr, "Name");
                result.Info.Alias = GetString(rdr, "Alias");
            }
    ...
    [HttpPost]
    public async Task<ActionResult<ResponseCallBack>> SaveAlias([FromBody] AliasData data, CancellationToken ct)
    {
    ...
    public async Task<ActionResult<List<AliasRecord>>> SearchAlias(int codetrans, CancellationToken ct)
    {
        try
        {
            await using var cn = new SqlConnection(ConnectionString);
            ...
            while (await rdr.ReadAsync(ct))
            {
                ...
                {
                    Code = GetInt(rdr, "Code"),
                    IdMain = GetInt(rdr, "IdMain"),
                    CodeTrans = GetInt(rdr, "CodeTrans"),
                    Name = GetString(rdr, "Name"),
                    Alias = GetString(rdr, "Alias")
                });
            }
    ...
    public async Task<ActionResult<List<AliasRecord>>> SearchAliasByName(int codetrans, string name = "", CancellationToken ct = default)
    {
        try
        {
    ...
    [HttpPost("search")]
    public async Task<ActionResult<List<AliasRecord>>> SearchAliasByName2([FromBody] ConfigurationcSearch data, CancellationToken ct)
    {
        try
        {
            await using var cn = new SqlConnection(ConnectionString);
    ...
    [HttpPost("delete")]
    public async Task<ActionResult<ResponseCallBack>> DeleteAlias([FromBody] GenericDeleteData data, CancellationToken ct)
    {
        var response = new ResponseCallBack();
        try
        {
            var codeList = string.Join(",", (data?.CodeList ?? new List<DeleteCode>())
                .Select(c => c.Code)
                .Distinct());
            ...
            if (rc != 0)
            {
                throw new InvalidOperationException($"[{rc}] Delete alias failed");
            }
    ...
    public async Task<ActionResult<ResponseCallBack>> PurgeAlias(int type, long codeUser, long codeSite, CancellationToken ct)
    {
        var response = new ResponseCallBack();
        try
        {
            ...
            if (rc != 0)
            {
                throw new InvalidOperationException($"[{rc}] Purge alias failed");
            }
