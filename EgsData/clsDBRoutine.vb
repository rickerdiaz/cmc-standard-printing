...
'Last Updated:
'Author: Jimson Ronald Lee
'About: Classes that interact with database inherit this class
'++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Imports System.Data
...
#Region "private utility methods & constructors"
    ...
    'to prevent instances from being created with "new SqlHelper()".
    ...
    Private Shared Sub AttachParameters(ByVal command As SqlCommand, ByVal commandParameters() As SqlParameter)
        ...
        For Each p In commandParameters
            'AGL 2013.03.16 - added checking for empty parameters
...
#Region "ExecuteNonQuery"
    ...
    Public Overloads Shared Function ExecuteNonQuery(ByVal connectionString As String, _
                                                 ByVal commandType As CommandType, _
                                                 ByVal commandText As String, _
                                                 ByVal commandParameters() As SqlParameter, ByVal commandTimeout As Integer) As Integer
        'create & open a SqlConnection, and dispose of it after we are done.
        Dim cn As New SqlConnection(connectionString)
        Try
            cn.Open()
            ...
            'call the overload that takes a connection in place of the connection string
            Return ExecuteNonQuery(cn, commandType, commandText, commandParameters, commandTimeout)
        Finally
            cn.Dispose()
        End Try
    End Function 'ExecuteNonQuery
    ...
    Public Overloads Shared Function ExecuteNonQuery(ByVal connectionString As String, _
                                                     ByVal commandType As CommandType, _
                                                     ByVal commandText As String, _
    ByVal oTransaction As SqlTransaction, ByVal ParamArray commandParameters() As SqlParameter) As Integer
        'create & open a SqlConnection, and dispose of it after we are done.
        Dim cn As New SqlConnection(connectionString)
        Try
            cn.Open()
            ...
            'call the overload that takes a connection in place of the connection string
            Return ExecuteNonQuery(cn, commandType, commandText, oTransaction, commandParameters)
        Finally
            cn.Dispose()
        End Try
    End Function 'ExecuteNonQuery
    ...
    Public Overloads Shared Function ExecuteNonQuery(ByVal connection As SqlConnection, _
        ...
        cmd.Dispose()
        connection.Close() 'DLS 27.01.2009
        connection.Dispose() 'DLS 27.01.2009
        Return retval
    End Function 'ExecuteNonQuery
    ...
    Public Overloads Shared Function ExecuteNonQuery(ByVal connection As SqlConnection, _
                                                     ByVal commandType As CommandType, _
                                                     ByVal commandText As String, _
                                                     ByVal oTrans As SqlTransaction, _
                                                     ByVal ParamArray commandParameters() As SqlParameter) As Integer
        ...
        PrepareCommand(cmd, connection, oTrans, commandType, commandText, commandParameters)
...
#Region "ExecuteDataset"
    ...
    Public Overloads Shared Function ExecuteDataset(ByVal connectionString As String, _
                                                ByVal commandType As CommandType, _
                                                ByVal commandText As String, _
                                                ByVal commandParameters() As SqlParameter, ByVal commandTimeOut As Integer) As DataSet
        'create & open a SqlConnection, and dispose of it after we are done.
        Dim cn As New SqlConnection(connectionString)
        Try
            cn.Open()
            ...
            'call the overload that takes a connection in place of the connection string
            Return ExecuteDataset(cn, commandType, commandText, commandParameters, commandTimeOut)
        Finally
            cn.Dispose()
        End Try
    End Function 'ExecuteDataset
...
#Region "EGS"
    ''' <summary>
    ''' Fetch Data using 3 different objects
    ''' </summary>
    ''' <param name="eFetchType"></param>
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Function ExecuteFetchType(ByVal eFetchType As enumEgswFetchType, ByVal strCnn As String, ByVal cmdType As CommandType, ByVal strCommandText As String, ByVal arrParam() As SqlParameter) As Object
        Try
    ...
    ''' <summary>
    ''' Fetch Data with Parameters required
    ''' </summary>
    ''' <param name="eFetchType"></param>
    ''' <param name="strCnn"></param>
    ''' <param name="cmdType"></param>
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Function ExecuteFetchType(ByVal eFetchType As enumEgswFetchType, ByVal strCnn As String, ByVal cmdType As CommandType, ByVal strCommandText As String) As Object
        Try
            Select Case eFetchType
                Case enumEgswFetchType.DataReader
                    Return ExecuteReader(strCnn, cmdType, strCommandText)
                Case enumEgswFetchType.DataSet
                    Return ExecuteDataset(strCnn, cmdType, strCommandText)
                Case enumEgswFetchType.DataTable
                    Return ExecuteDataset(strCnn, cmdType, strCommandText).Tables(0)
            End Select
            Return Nothing
        Catch ex As Exception
            Throw ex
    ...
    Function ExecuteFetchType(ByVal eFetchType As enumEgswFetchType, ByRef sqlCmd As SqlCommand) As Object
        Try
...
#End Region
    ...
    Public Sub SaveError(ByVal codeuser As Integer, _
                         ByVal value As String, _
                         ByVal sLogPageError As String, _
                         ByVal sLogExceptionType As String, _
                         ByVal sLogStackTrace As String, _
                         ByVal sLogSource As String, _
                         ByVal sLogTargetSite As String, _
                         ByVal sRequestedURL As String, _
                         ByVal strcnn As String, _
                         ByRef intCode As Integer)

        Try
            Dim cn As New SqlConnection(strcnn)
            Dim cmd As New SqlCommand
            Dim da As New SqlDataAdapter
            Dim dt As New DataSet

            sLogStackTrace = Replace(sLogStackTrace, vbCrLf, "")
            value = Replace(value, vbCrLf, "")

            With cmd
                .Connection = cn
                .CommandText = "sp_EgswAddErrorMessage"
                .CommandType = CommandType.StoredProcedure
                .Parameters.Add("@p_nCodeUser", SqlDbType.Int).Value = codeuser
                .Parameters.Add("@p_nValue", SqlDbType.VarChar, 5000).Value = Left(value, 2000)
                .Parameters.Add("@p_sLogPageError", SqlDbType.VarChar, 100).Value = Left(sLogPageError, 100)
                .Parameters.Add("@p_sLogExceptionType", SqlDbType.VarChar, 100).Value = Left(sLogExceptionType, 100)
                .Parameters.Add("@p_sLogStackTrace", SqlDbType.VarChar, 2000).Value = Left(sLogStackTrace, 5000)
                .Parameters.Add("@p_sLogSource", SqlDbType.VarChar, 100).Value = Left(sLogSource, 100)
                .Parameters.Add("@p_sLogTargetSite", SqlDbType.VarChar, 100).Value = Left(sLogTargetSite, 100)
                .Parameters.Add("@p_sRequestedURL", SqlDbType.VarChar, 250).Value = Left(sRequestedURL, 250)
                .Parameters.Add("@p_nCode", SqlDbType.Int).Direction = ParameterDirection.Output
                '
                .Connection.Open()
                .ExecuteNonQuery()
                .Connection.Close()
