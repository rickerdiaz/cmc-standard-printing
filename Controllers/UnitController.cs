...
namespace CalcmenuAPI
{
    ...
    public class UnitController : ControllerBase
    {
        ...
        public ActionResult<List<Models.GenericList>> GetUnitByKind(int codesite, int codetrans, int kind)
        {
            try
            {
                var ds = new DataSet();
                ...
                {
                    list.Add(new Models.GenericList
                    {
                        Code = GetInt(r["Code"]),
        ...
        public ActionResult<List<Models.Unit>> GetUnit(int codesite, int codetrans, int codesetprice, int mainType, int type = (int)UnitType.Neutral, int codeliste = -1)
        {
            try
            {
                var ds = new DataSet();
                ...
                var list = new List<Models.Unit>();
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    list.Add(new Models.Unit
                    {
                        ...
                        IsIngredient = GetInt(dr["IsIngredient"]),
        ...
        public ActionResult<Models.ResponseCallBack> GetUnitByName(int codesite, string name = "")
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(name))
                {
                    cmd.Connection = cn;
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandText = SP_API_GET_UnitByName; // assumes constant is defined elsewhere
        ...
        public ActionResult<List<Models.UnitInfo>> GetUnitInfo(int codeunit, int codetrans)
        {
            try
            {
                var ds = new DataSet();
                ...
                var list = new List<Models.UnitInfo>();
                foreach (DataRow r in ds.Tables[0].Rows)
                {
                    list.Add(new Models.UnitInfo
                    {
                        Code = GetInt(r["Code"]),
        ...
        public ActionResult<Models.UnitConvert> ConvertUnit(int codesite, int codetrans, int conversiontype, int codeunit, int useplural, double value1 = 0.0, double value2 = 0.0)
        {
            try
            {
                var unit = new Models.UnitConvert
                {
                    Code = codeunit,
                    Name = string.Empty,
                    Value1 = value1,
        ...
        public ActionResult<List<Models.Unit>> SearchUnitByName(int codesite, int codetrans, int type, int status, int codeproperty = -1, int merchandiseyield = 0, string name = "")
        {
            try
            {
                var ds = new DataSet();
                using var cmd = new SqlCommand();
                using var cn = new SqlConnection(ConnectionString);
                cmd.Connection = cn; // original VB used ConfigurationManager dsn
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                if (!string.IsNullOrWhiteSpace(name))
                {
                    var unitresult = new List<Models.Unit>();
        ...
        public ActionResult<Models.UnitResponseSearch> SearchUnitByName2([FromBody] Models.ConfigurationcSearch data)
        {
            try
            {
                var ds = new DataSet();
                ...
                return Ok(new Models.UnitResponseSearch(units, totalCount));
            }
            catch (HttpResponseException) { throw; }
            catch (Exception) { return StatusCode(500); }
        }
        ...
        public ActionResult<List<Models.UnitTranslation>> GetUnitTranslation(int codeunit, int codesite)
        {
            try
            {
                var ds = new DataSet();
                ...
                var list = new List<Models.UnitTranslation>();
                foreach (DataRow r in ds.Tables[0].Rows)
                {
                    list.Add(new Models.UnitTranslation
                    {
        ...
        public ActionResult<List<Models.TreeNode>> GetUnitSharing(int codesite, int type, int tree, int codeunit)
        {
            try
            {
                var ds = new DataSet();
        ...
        [HttpPost("api/unit")]
        public ActionResult<Models.ResponseCallBack> SaveUnit([FromBody] Models.UnitData data)
        {
            var response = new Models.ResponseCallBack();
            int resultCode = 0;
        ...
        public ActionResult<Models.ResponseCallBack> DeleteUnit([FromBody] Models.GenericDeleteData data)
        {
            var response = new Models.ResponseCallBack();
            int resultCode = 0;
            try
            {
                ...
                var joined = string.Join(",", codes); // VB passes CSV without parentheses
        ...
        public ActionResult<Models.ResponseCallBack> ActivateDeactivate([FromBody] Models.ActivateDeactivate data)
        {
            var response = new Models.ResponseCallBack();
            int resultCode = 0;
            SqlTransaction trans = null;
